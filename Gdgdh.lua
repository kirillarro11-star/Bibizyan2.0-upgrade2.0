-- ============================================================
--           BIBIZZZAN HUB  v3.0  (FINAL, STABLE)
...
-- ============================================================
--  ОСНОВНОЕ МЕНЮ (размер ~7.5 см → 310x470)
-- ============================================================
...
-- Перетаскивание меню (аналогично кнопке)
local dragMain = false
local dragStartMain, startPosMain, hasMovedMain
...
UserInputService.InputChanged:Connect(function(input)
    if dragMain and (input.UserInputType == Enum.UserInputType.Touch or input.UserInputType == Enum.UserInputType.MouseMovement) then
        local delta = (input.Position - dragStartMain).Magnitude
        if delta > 10 then
            hasMovedMain = true
            MainFrame.Position = UDim2.new(
                startPosMain.X.Scale, startPosMain.X.Offset + (input.Position.X - dragStartMain.X),
                startPosMain.Y.Scale, startPosMain.Y.Offset + (input.Position.Y - dragStartMain.Y)
            )
        end
    end
end)
...
-- ============================================================
local espObjects = {}   -- все объекты рисования (Text, Line, коннекты)
local lineObjects = {}
...
-- Запускаем обновление ESP в цикле
...
local function applyCharacter(char)
    ...
    if Settings.Aimbot or Settings.SilentAim then
        aimbotConn = RunService.RenderStepped:Connect(function()
            ...
            -- Приблизительный перевод градусов в пиксели (зависит от FOV, но для простоты так)
...
print("✅ BIBIZZZAN HUB v3.0 загружен! Нажмите кнопку для открытия меню.")
