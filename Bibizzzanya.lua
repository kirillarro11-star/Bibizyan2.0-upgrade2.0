...
-- Логика открытия/закрытия с защитой от перетаскивания
...
ToggleButton.InputEnded:Connect(function(input)
    if input.UserInputType == Enum.UserInputType.Touch or input.UserInputType == Enum.UserInputType.MouseButton1 then
        ...
        -- Если не было перемещения – это клик
...
-- Перетаскивание меню (аналогично)
local dragMain = false
local dragStartMain, startPosMain, hasMovedMain = false
...
local function applyCharacter(char)
    ...
    if Settings.Aimbot or Settings.SilentAim then
        aimbotConn = RunService.RenderStepped:Connect(function()
            ...
            local radiusPx = Settings.SilentAimRadius * 3  -- примерный перевод
...
print("✅ bibizzzan Hub v2.1 загружен! Нажмите кнопку для открытия.")
