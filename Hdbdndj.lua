...
-- ============================================================
--  ОСНОВНОЕ МЕНЮ (создаём ПЕРВЫМ, чтобы обработчики ссылались)
-- ============================================================
...
-- ============================================================
--  КНОПКА ОТКРЫТИЯ (исправленная логика)
-- ============================================================
...
-- Логика: короткое нажатие = открыть/закрыть, движение = перетаскивание
...
-- ============================================================
--  КОНТЕЙНЕР ДЛЯ ЭЛЕМЕНТОВ (скроллинг)
-- ============================================================
...
local function addY(offset)
    Content.CanvasSize = UDim2.new(0,0,0,Content.CanvasSize.Y.Offset + offset)
end
...
speedFill.Size = UDim2.new(Settings.SpeedValue/100, 0, 1, 0)
...
local function applyCharacter(char)
    ...
    local speedConn = RunService.Stepped:Connect(function()
        hum.WalkSpeed = Settings.FastSpeed and Settings.SpeedValue or 16
    end)
...
local dragStartMenu, startPosMenu, hasMovedMenu
...
UserInputService.InputChanged:Connect(function(input)
    if dragMenu and (input.UserInputType == Enum.UserInputType.Touch or input.UserInputType == Enum.UserInputType.MouseMovement) then
        local delta = (input.Position - dragStartMenu).Magnitude
        if delta > 10 then
            hasMovedMenu = true
            MainFrame.Position = UDim2.new(
                startPosMenu.X.Scale, startPosMenu.X.Offset + (input.Position.X - dragStartMenu.X),
                startPosMenu.Y.Scale, startPosMenu.Y.Offset + (input.Position.Y - dragStartMenu.Y)
            )
        end
    end
end)
...
print("✅ BIBIZAN HUB v3.0 загружен! Нажмите кнопку 'bibizan Hub' для открытия меню.")
