-- ============================================================
--           BIBIZZZAN HUB  v6.0  (FINAL STABLE)
...
-- ============================================================
--  ОСНОВНОЕ МЕНЮ (размер ~6.5 см → 270x420)
-- ============================================================
...
-- ============================================================
--  КНОПКА ОТКРЫТИЯ (перемещаемая, с защитой от случайного клика)
-- ============================================================
...
yEsp = yEsp + 32
makeToggle(espFrame, "Box ESP", yEsp, function() return Settings.Box end, function(v) Settings.Box = v end)
...
local function updateESP()
    clearESP()
    if not Settings.Esp and not Settings.Lines and not Settings.Box then return end
    ...
    for _, p in ipairs(players) do
        ...
        if Settings.Box then
            ...
            table.insert(boxObjects, conn)
        end
    end
end

local espUpdater = RunService.RenderStepped:Connect(function()
    if Settings.Esp or Settings.Lines or Settings.Box then
        updateESP()
    else
        clearESP()
    end
...
local function applyCharacter(char)
    ...
    local jumpConn = RunService.Stepped:Connect(function()
        if Settings.HighJump or Settings.InfinityJump then
    ...
    -- Infinity Jump (повторный прыжок в воздухе)
    local infJumpConn
    ...
    if Settings.Aimbot or Settings.SilentAim then
        aimbotConn = RunService.RenderStepped:Connect(function()
            ...
            local radiusPx = Settings.SilentAimRadius * 3   -- приблизительный перевод
    ...
    -- Сохраняем все соединения для очистки при смене персонажа
...
print("✅ BIBIZZZAN HUB v6.0 загружен! Нажмите кнопку для открытия.")
